...
const byte adcInput = 4;          //input pin for analog read
int ohmValue;
...
void loop() {
  ...
  for (int i = 110; i > 15; i--) {
    setPotWiper(pot, i);
    ohmValue  = analogRead(adcInput);
    Serial.print("Value read from the analog read: ");
    Serial.println(analInput);
    Serial.print("Value read from the analog read converted ");
    Serial.println(ohmValue/4096*rAB);
    delay(50);
  }
